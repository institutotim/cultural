...
add_action('admin_head', 'add_scripts_admin');
...
//        if(is_home() || is_archive() && get_post_type() === 'evento'){
		...
		wp_enqueue_script('angular-daterangepickerHome', get_bloginfo('template_directory') . '/js/lib/angular-daterangepicker-home.js', array('angular-core'), null, false);
...
register_sidebar(array(
'name' => 'video',
'before_widget' => '<div class="navbar_ccsp">',
'after_widget' => '</div>',
'before_title' => '<h2>',
'after_title' => '</h2>',
));
...
//TAMANHO DO RESUMO DO SLIDER

function resumo_slider($length) {
    return 20;
}
add_filter('excerpt_length', 'resumo_slider');
