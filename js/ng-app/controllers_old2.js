(function(angular){
    ...
    app.controller('eventsController', ['$rootScope', '$scope', '$log', '$location', '$timeout', 'searchService', '$sce', '$http', '$q', 'buscaEspacosService', 'buscaProjetosService', function(
                                            $rootScope,   $scope,   $log,   $location,   $timeout,   searchService,   $sce, $http,  $q, buscaEspacosService, buscaProjetosService){
						
		buscaEspacosService.getSpaces()
			.then(
			function(result) {
			$scope.data.spaces = result;
		  },
		  function(error) {
		...
		/*$http.get('http://spcultura.prefeitura.sp.gov.br/api/space/find?@select=id,name').success(function ( data) {
       		$scope.data.spaces = JSON.stringify(data);
       		alert($scope.data.spaces);
        });
        */
