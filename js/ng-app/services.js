(function(angular){
    var app = angular.module('CulturalTheme');
     ...
     app.service("buscaEspacosService", function($http, $q) {
	  ...
	  this.getSpaces = function() {
		return $http.get('http://spcultura.prefeitura.sp.gov.br/api/space/find?@select=id,name,isVerified&&@order=name ASC')
	...
	app.service("buscaEnderecoEspaco", function($http, $q) {

	  var deferred = $q.defer();

	  this.getEnderecos = function() {
            return $http.get('http://spcultura.prefeitura.sp.gov.br/api/space/find?@select=id,endereco')
		  .then(function(response) {
			// promise is fulfilled
			deferred.resolve(response.data);
			return deferred.promise;
	...
	app.service("buscaProjetosService", function($http, $q) {
	  ...
	  this.getProjects = function() {
		return $http.get('http://spcultura.prefeitura.sp.gov.br/api/project/find?@select=id,name,isVerified&&@order=name ASC')
		  .then(function(response) {
			// promise is fulfilled
			deferred.resolve(response.data);
			return deferred.promise;
    ...
    app.factory('searchService', ['$rootScope', '$q', '$http', '$log', function($rootScope, $q, $http, $log) {
        ...
        svc.submit = function(){
            var deferred = $q.defer();
            var searchParams = {
                '@select': 'id,singleUrl,name,subTitle,type,shortDescription,terms,classificacaoEtaria,traducaoLibras,descricaoSonora,owner,project.name,project.singleUrl,occurrences',
               // '@page': 1,
               // '@limit': 15,
			...
			//Espacos
			if(svc.data.spaces && svc.data.spaces.length){
                searchParams['space'] = "IN(";
                for(var i= 0; i < svc.data.spaces.length; i++){
					if(i == svc.data.spaces.length - 1){
						searchParams['space'] += "@Space:" + svc.data.spaces[i].id;
					}else{
						searchParams['space'] += "@Space:" + svc.data.spaces[i].id+",";
					}
				}
            ...
            // ou se está numa categoria tem filtro configurado para a mesma
            }else if(vars.categoryFilters && vars.categoryFilters.space){
            ...
            if(svc.data.projects && svc.data.projects.length){
                searchParams['project'] = "IN(";
                for(var i= 0; i < svc.data.projects.length; i++){
					if(i == svc.data.projects.length - 1){
						searchParams['project'] += "@Project:" + svc.data.projects[i].id;
					}else{
						searchParams['project'] += "@Project:" + svc.data.projects[i].id+",";
					}
				}
            ...
            // ou se está numa categoria tem filtro configurado para a mesma
            }else if(vars.categoryFilters && vars.categoryFilters.project){
			...
			svc.data.initialSearch = svc.data.initialSearch+1;
