(function() {
  ...
  picker = angular.module('daterangepickerHome', []);

  picker.value('dateRangePickerHomeConfig', {
	 autoApply: true,
	 singleDatePicker: true,
	 id:'dateRangePickerHome',
	 buttonClasses:'hideBtn',
	 ranges: {
           'Hoje': [moment(), moment()],
           'Amanhã': [moment().add(1, 'days'), moment().add(1, 'days')],
           'Este Fim de Semana': [moment().day(6), moment().day(7)],
           'Este Mês': [moment().startOf('month'), moment().endOf('month')],
    }
  });

  picker.directive('dateRangePickerHome', [
    '$compile', '$timeout', '$parse', 'dateRangePickerHomeConfig', function($compile, $timeout, $parse, defaults) {
      return {
        require: 'ngModel',
        restrict: 'A',
